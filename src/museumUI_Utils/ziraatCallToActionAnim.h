...
public:
	...
	void drawTextMessage(string _s, ofTrueTypeFont *_f, int _yPos = 800) {
		font = _f;

		// Info Text
		int mfw = font->getStringBoundingBox(_s, 0, 0).getWidth();

		// Input Text Draw
		ofPushStyle();
		ofSetColor(ofColor::red, 200);
		font->drawString(_s, ofGetWidth()*0.5 - mfw * 0.5, _yPos);
		ofPopStyle();
	}
