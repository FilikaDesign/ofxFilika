...
public:
	...
	void threadedFunction()
	{
		...
		message->addContent(new Poco::Net::StringPartSource(this->contentText));
		...
		try
		{
			...
			/*message->addAttachment(Poco::Net::MailMessage::encodeWord(koko, "UTF-8"),
				new Poco::Net::FilePartSource(ofToDataPath(koko)));*/
		...
		std::string html = "<html><body><br><img src=\"cid:image\"></body></html>";
		message->addPart("", new Poco::Net::StringPartSource(html, "text/html"),Poco::Net::MailMessage::CONTENT_INLINE, Poco::Net::MailMessage::ENCODING_BASE64);
	...
	void sendMail(string mailTo, string fileName, string contentTxt="") {

		//senderInfo = "Herr Artut<" + mailTo + ">";
		//C = senderInfo.c_str(); // Sender Info

		senderEmailAddress = mailTo;
		this->contentText = contentTxt;
	
		//imgPath = "D:\\Development\\of_v0.11.2_vs2017_release\\apps\\ziraat_Izmir_Projects\\ziraat_GreenScreen_main\\bin\\data\\" + fileName;// + dirName + "\\" + fileName;
		imgPath = fileName;
		startThread();
		stopThread();
