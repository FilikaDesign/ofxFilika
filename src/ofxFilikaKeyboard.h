...
public:
	...
	void setup(int x, int y, int w, string fontPath, string _lang = "_TR") {
		lang = _lang;
		fontKeySize = w*0.03f;
		...
		loadKeys();
		inputField.setup(x, y*0.5f, w, h*0.3f, h*0.1f, *fontOutput, colorText, lang);
	}
	void display() {
		ofPushStyle();
		if (active) {
...
private:
	...
	void loadKeys() {
		...
		if(lang == "_TR")
			...
			keys[64].setup("SEND", "SEND", *fontKey, x + margin * 11 + keySize * 10, y + margin * 4 + keySize * 3, keySize * 2 + margin, keySize);
